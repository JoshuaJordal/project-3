...
def check():
    ...
    if matched and in_jumble and not (text in matches):
        ...
        if len(matches) >= flask.session["target_count"]:
            rslt = {"text": "You found {}".format(text), "matched": True, "matches": matches, "redirect": True}
            return flask.jsonify(result=rslt)
        
        rslt = {"text": "You found {}".format(text), "matched": True, "matches": matches}
        return flask.jsonify(result=rslt)
    elif text in matches:
        rslt = {"text": "You already found {}".format(text), "matched": False}
        return flask.jsonify(result=rslt)
    elif not matched:
        rslt = {"text": "{} isn't in the list of words".format(text), "matched": False}
        return flask.jsonify(result=rslt)
    elif not in_jumble:
        rslt = {"text": '"{}" can\'t be made from the letters {}'.format(text, jumble), "matched": False}
        return flask.jsonify(result=rslt)
    else:
